import { group, index, map, unique, filter, metadata, toUnderscored } from '../core'
import { settings } from './settings'


export const migrate = async () => {
    const database = settings.db
    const newSchema = map(group(filter(metadata, (_, metadata) => metadata.type === 'persisted'), (_, d) => d.className.toLowerCase()), (_, metadata) => index(metadata, (_, metadata) => toUnderscored(metadata.key)))
