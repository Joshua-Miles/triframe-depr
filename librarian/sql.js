...
export async function query(callback, { disableLogging = false } = {}) {
    ...
    const crawl = (record) => {
        if (!record) {
            ...
            if (record[0] && record[0].__isOnly__) {
                if (record[0].id) {
            ...
            if (record.__of__ || (record[0] && record[0].id)) {
    ...
    const escape = attribute => {
        if (Array.isArray(attribute)) attribute = JSON.stringify(attribute)
    ...
    const createLibrary = models => {
        ...
        const library = {
            ...
            eachAttribute: (attributes, callback, seperator = ',') => {
                let result = ''
                each(attributes, (key, value) => {
                    if (value === undefined) return
